...
/*
 * SPI communication between STM32(Master) and Arduino(Slave)
 * Software works in a way:
 * 1. User enters the message using Arduino serial monitor terminal
 * 2. Arduino notifies the STM32 board about message availability
 * 3. STM32 device reads and prints the message
 *
 * Interrupt based communication, as there is a communication over 4 SPI lines, but also there is a GPIO pin from
 * Arudino (PIN8) to STM32 (PD6), that would notify STM32 of a received message
 */
